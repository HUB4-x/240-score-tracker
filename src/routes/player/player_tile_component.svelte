<script lang="ts">
    ...
    let { player = $bindable() as Player, index=undefined, editFunction_parent = undefined, updateFunction = undefined, onClick = undefined} = $props<{
        player: Player,
        index?: number | null | undefined,
        editFunction_parent?: (p: Player)=>void | null,
...
<div class="w-full h-full flex rounded rounded-lg bg-base-300 p-2 gap-x-3 min-w-70 overflow-hidden group" on:click={()=>{onClick? onClick() : console.log('FAIL')}}>
    ...
    <div class="w-full h-full flex overflow-hidden">
        <div class="flex flex-col max-w-full w-full overflow-hidden">
            ...
            <div class="w-full mt-auto flex flex-col gap-y-1">
                <p class="text-md w-full mt-auto">Number of 180:  {player.stats && player.stats.number_of_240? player.stats.number_of_240 : 0}</p>
